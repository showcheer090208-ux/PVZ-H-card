...
class ComponentParamDialog(QDialog):
    def __init__(self, node_id, current_params, parent=None):
        ...
        self.widgets = {}
        self.schema = logic_library.NODE_DEF[node_id].get("editable_params", {})
        ...
        for key, config in self.schema.items():
            ...
            if config["type"] == "int":
                ...
                w = QComboBox()
                for opt in config["options"]: w.addItem(localization.ENUM_NAMES.get(opt, opt), opt)
        ...
        if node_id == "PrimaryTargetFilter" and "SelectionType" in self.widgets and "NumTargets" in self.widgets:
            ...
            def on_sel_change(index):
                ...
                if text in ["All", "Manual"]: num_spin.setValue(0)
